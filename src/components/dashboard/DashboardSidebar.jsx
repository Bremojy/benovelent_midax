...
function DashboardSidebar({ role, sidebarOpen, setSidebarOpen, mobileHidden = false, mobileBottomNav = false }) {
  ...
  const mobilePrimaryItems = (() => {
    if (!mobileBottomNav) return menu;
  ...
  const mobileMoreItems = menu.filter((item) => !mobilePrimaryItems.some((primary) => primary.path === item.path));
  ...
  return (
    <aside className={`${sidebarOpen ? "dashboard-sidebar open" : "dashboard-sidebar"} ${mobileHidden ? "mobile-hidden" : ""} ${mobileBottomNav ? "mobile-bottom-nav" : "mobile-drawer"}`} aria-label="Dashboard navigation">
      ...
      <nav className="sidebar-menu" aria-label={`${roleLabel} navigation`}>
        {(mobileBottomNav ? mobilePrimaryItems : menu).length > 0
          ? (mobileBottomNav ? mobilePrimaryItems : menu).map((item) => {
              const Icon = item.icon;
              return (
                <NavLink key={`${item.title}-${item.path}`} to={item.path} onClick={handleNavigation} className={({ isActive }) => (isActive ? "sidebar-link active" : "sidebar-link")}>
        ...
        {mobileBottomNav && mobileMoreItems.length > 0 && (
          <button type="button" className={`sidebar-link sidebar-more-button ${moreOpen ? "active" : ""}`} onClick={() => setMoreOpen((value) => !value)} aria-expanded={moreOpen} aria-label="More dashboard sections">
      ...
      {mobileBottomNav && moreOpen && (
        <div className="mobile-more-panel" role="dialog" aria-label="More dashboard sections">
          ...
          <div className="mobile-more-grid">
            ...
            <button type="button" className="mobile-more-link logout-more" onClick={logout}><LogOut size={19} /><span>Logout</span></button>
          </div>
        </div>
      )}

      {!mobileBottomNav && <div className="sidebar-footer"><button type="button" className="logout-btn" onClick={logout}><LogOut size={20} strokeWidth={2} /><span>Logout</span></button></div>}
    </aside>
  );
}
