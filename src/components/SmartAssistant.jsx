...
const FAQ = [
  ...
  { keys: ["superadmin", "super admin", "super administrator"], answer: "The superadmin portal contains higher-level administration, system, audit, data-integrity, member/admin management, notifications, news, feedback, finance and settings tools. SuperAdmin does not use the member/admin chat area." },
  ...
  { keys: ["login loop", "logged out", "login keeps", "sign me out", "logout loop"], answer: "If the portal keeps returning you to the login page, first refresh once and sign in again. Benevolent MIDAX now protects against stale session checks and temporary server wake-ups. If it continues, use the same browser account consistently and contact an administrator rather than repeatedly submitting the password." },
  { keys: ["slow website", "website slow", "loading slow", "performance", "fast", "faster"], answer: "Benevolent MIDAX is optimized for compressed responses, cached public data and Redis when configured. A first request after the server has been idle can still take longer because the backend may need to wake up; subsequent requests should be faster." },
