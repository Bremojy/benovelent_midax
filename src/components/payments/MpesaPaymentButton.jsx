...
export default function MpesaPaymentButton({ purpose, referenceId, label = "Pay with M-PESA", defaultAmount = "", maxAmount = 0, phoneNumber = "", disabled = false, onSuccess }) {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        if (attempts >= 30) {
          setStatus("timeout");
          setMessage("M-PESA confirmation is taking longer than expected. No payment has been reported as successful yet; the server will continue processing the callback.");
          return;
        }
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      setStatus("sent"); setMessage(data?.message || "STK request sent. Check your phone and enter your M-PESA PIN. Final payment status will be confirmed by the server.");
    } catch (error) {
        ...
        <div className="mpesa-modal-backdrop" role="presentation" onMouseDown={(e) => { if (e.target === e.currentTarget) setOpen(false); }}>
          <section className="mpesa-modal" role="dialog" aria-modal="true" aria-label="M-PESA payment">
            ...
            <form onSubmit={submit} className="mpesa-form">
              ...
              <div className="mpesa-account-note">Scheme collection: PayBill <strong>{mpesaConfig.manualPaybill || "Not configured"}</strong>{mpesaConfig.manualAccountNumber ? <> • Manual Account <strong>{mpesaConfig.manualAccountNumber}</strong></> : null}</div>
