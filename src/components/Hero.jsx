...
function buildImageUrl(value, version) {
  ...
  const isAbsolute = /^(https?:|blob:|data:)/i.test(raw);
  const isLocalPublicAsset = raw.startsWith("/") && !raw.startsWith("/uploads/") && !raw.startsWith("/documents/");
  const base = isAbsolute || isLocalPublicAsset ? raw : resolveUploadUrl(raw);
