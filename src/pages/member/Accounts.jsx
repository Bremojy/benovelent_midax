...
export default function MemberAccounts() {
  ...
  return <DashboardLayout><main className="portal-page account-page">
    ...
    {tab === "constitution" && <>
      <section className="account-grid four"><div className="portal-panel account-card"><span>Current Constitution Book Balance</span><strong>{money(bookBalance?.balance)}</strong><small>As of {bookBalance?.asOf ? new Date(bookBalance.asOf).toLocaleString("en-KE") : "the latest recorded book calculation"}</small><div className="account-live-flow" style={{display:"flex",gap:12,flexWrap:"wrap",marginTop:8}}><small>Money in: {money(bookBalance?.moneyIn)}</small><small>Money out: {money(bookBalance?.moneyOut)}</small></div></div><div className="portal-panel account-card"><span>Your contributions</span><strong>{money(summary?.totalContributed)}</strong><small>Recorded member contributions</small></div><div className="portal-panel account-card"><span>Contribution status</span><strong>{summary?.contributionStatus || "Not available"}</strong><small>Your personal contribution position</small></div><div className="portal-panel account-card"><span>What this balance means</span><strong style={{fontSize:14}}>Scheme funds</strong><small>Based on approved financial transactions</small></div></section>
