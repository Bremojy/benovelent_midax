...
import { ChevronLeft, ChevronRight, ExternalLink, MessageSquarePlus, Star, Trash2, Plus, X, Home, UserRound, Clock3, MessageCircle, Download, Upload, Newspaper, Printer } from "lucide-react";
...
export default function Feedback() {
  ...
  const saveBlob = (blob, filename) => { const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = filename; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url); };
  const downloadResponses = async (item, format) => { try { const r = await exportFeedbackResponses(item._id, format); saveBlob(r.data, `feedback-${String(item.title).replace(/[^a-z0-9]+/gi,"-").toLowerCase()}.${format}`); toast.success(`Responses downloaded as ${format.toUpperCase()}`); } catch (e) { toast.error(e.response?.data?.message || "Could not download responses"); } };
  const importResponses = async (item, e) => { const file = e.target.files?.[0]; if (!file) return; try { const r = await importFeedbackResponses(item._id, file, "append"); toast.success(`${r.data.imported} response${r.data.imported === 1 ? "" : "s"} imported`); load(); } catch (err) { toast.error(err.response?.data?.message || "Could not import responses"); } finally { e.target.value = ""; } };
  const publishReport = async (item) => { try { const r = await publishFeedbackToNews(item._id); toast.success(r.data.updated ? "Feedback report updated in News." : "Feedback report published to News."); load(); } catch (e) { toast.error(e.response?.data?.message || "Could not publish feedback report"); } };
  ...
  return <div className="portal-page feedback-page">
    ...
    {loading ? <div className="feedback-skeleton">Loading feedback…</div> : <div className="feedback-card-grid">{items.map(item => <article className="interactive-card feedback-card" key={item._id}>
      {canManage && <div className="feedback-card-status" aria-label={`Status: ${item.status || "active"}`}>{String(item.status || "active").toUpperCase()}{item.promptOnLogin ? " · LOGIN PROMPT" : ""}{item.requireOnLogin ? " · REQUIRED" : ""}</div>}
      ...
      <div className="feedback-card-actions">{item.kind === "google_form" ? <a className="btn btn-primary" href={item.googleFormUrl} target="_blank" rel="noreferrer">Open form <ExternalLink size={16} /></a> : <button className="btn btn-primary" type="button" onClick={() => { setAnswers({}); setStep(0); setActive(item); }}>Give feedback</button>}{canManage && <button className="btn btn-secondary" type="button" onClick={() => openResponses(item)}>Responses ({item.responseCount || 0})</button>}{canManage && <button className="btn btn-secondary" type="button" onClick={() => downloadResponses(item,"csv")}><Download size={16}/> CSV</button>}{canManage && <button className="btn btn-secondary" type="button" onClick={() => downloadResponses(item,"json")}><Download size={16}/> JSON</button>}{canManage && <label className="btn btn-secondary feedback-upload-btn"><Upload size={16}/> Import<input type="file" accept=".csv,.json,text/csv,application/json" onChange={(e) => importResponses(item,e)} /></label>}{canManage && <button className="btn btn-primary" type="button" onClick={() => publishReport(item)}><Newspaper size={16}/> {item.publishedToNews ? "Update News" : "Publish to News"}</button>}</div>
    </article>)}</div>}
