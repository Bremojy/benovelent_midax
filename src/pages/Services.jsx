...
function policyDescription(policy) { const parts=[]; if(policy.minAmount!==null&&policy.minAmount!==undefined) parts.push(`Minimum: Ksh ${Number(policy.minAmount).toLocaleString("en-KE")}`); if(policy.maxAmount!==null&&policy.maxAmount!==undefined&&Number(policy.maxAmount)>0) parts.push(`Maximum: Ksh ${Number(policy.maxAmount).toLocaleString("en-KE")}`); if(policy.repaymentEnabled) parts.push(`Repayment: ${Number(policy.repaymentMonths || 12)} months`); return parts.join(" • ") || policy.description || "Current policy details are maintained by SuperAdmin."; }
