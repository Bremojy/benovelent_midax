...
exports.createMember = async (req, res) => {
  try {
    ...
    // ==========================================
    // BENOVELENT MIDAX MEMBER NUMBER
    // ==========================================
    // Member numbers are generated by the server. Client-supplied legacy
    // values such as employee numbers (for example `101`) are deliberately
    // ignored so the database can never receive an invalid membership ID.
    const cleanMemberNumber = await generateMemberNumber();
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message:
          "Full Name, Phone and Email are required for portal access. Benevolent MIDAX Member Number is generated automatically.",
      });
    }
...
exports.verifyMember = async (req, res) => {
  try {
    let member = await Member.findOne({ _id: req.params.id, role: "member", isDeleted: false });
    ...
    // Older records may still contain the former employee number (for example
    // `101`). The current schema intentionally rejects those values. Repair the
    // identifier atomically before saving the verified member.
    const legacyMemberNumber = normalizeLegacyMemberNumber(member.memberNumber);
    if (!legacyMemberNumber) {
      const replacementNumber = await generateMemberNumber();
      await Member.collection.updateOne(
        { _id: member._id },
        { $set: { memberNumber: replacementNumber } }
      );
      member.memberNumber = replacementNumber;
      member = await Member.findById(member._id);
      if (!member) return res.status(404).json({ success: false, message: "Member record disappeared during verification." });
    }
