...
const ASSISTANT_KNOWLEDGE = [
  ...
  ["login troubleshooting", /\b(login loop|logged out|keeps logging me out|cannot stay logged in|login problem|sign in problem|portal slow)\b/i, "For a login loop, refresh the portal once and sign in again. The application treats the REST authentication session as the source of truth and ignores stale background checks and temporary server wake-ups. Do not repeatedly submit passwords; if the issue continues, contact an authorised administrator."],
  ["performance", /\b(slow|faster|performance|speed|loading)\b/i, "Benevolent MIDAX uses response compression, cached application knowledge and optional Redis/Upstash caching. The first request after an idle backend can still be slower while the hosting service wakes; repeated requests should benefit from caching."],
