...
const isConfigured = () => {
  const enabled = env("MPESA_ENABLED", "true").toLowerCase() !== "false";
  return enabled && Boolean(env("MPESA_CONSUMER_KEY") && env("MPESA_CONSUMER_SECRET") && env("MPESA_PASSKEY") && env("MPESA_SHORTCODE", "247247") && env("MPESA_CALLBACK_URL"));
};
...
async function getAccessToken() {
  if (!isConfigured()) throw new Error("M-PESA Daraja is not configured on the server.");
...
async function stkPush({ phoneNumber, amount, accountReference, transactionDesc }) {
  ...
  const shortcode = env("MPESA_SHORTCODE", "247247");
  ...
  const payload = {
    ...
    AccountReference: String(accountReference || env("MPESA_ACCOUNT_REFERENCE", "0650186528835")).slice(0, 20),
...
async function b2cPayment({ phoneNumber, amount, remarks, occasion }) {
  const required = ["MPESA_CONSUMER_KEY", "MPESA_CONSUMER_SECRET", "MPESA_INITIATOR_NAME", "MPESA_SECURITY_CREDENTIAL", "MPESA_B2C_RESULT_URL", "MPESA_B2C_TIMEOUT_URL"];
  if (required.some((name) => !env(name))) throw new Error("M-PESA B2C payout is not fully configured on the server.");
  const accessToken = await getAccessToken();
  const response = await axios.post(`${baseUrl()}/mpesa/b2c/v1/paymentrequest`, {
    ...
    Amount: Math.max(1, Math.round(Number(amount))),
    PartyA: env("MPESA_B2C_SHORTCODE", env("MPESA_SHORTCODE", "247247")),
...
module.exports = { isConfigured, normalizePhone, stkPush, b2cPayment, idempotencyKey };
