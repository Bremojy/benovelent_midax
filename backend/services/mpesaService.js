...
const maskPhone = (value) => {
  ...
  return /^254\d{9}$/.test(normalized) ? `${normalized.slice(0, 5)}*****${normalized.slice(-2)}` : "masked";
};
const isDarajaConfigured = () => Boolean(
  !isPlaceholder(env("MPESA_CONSUMER_KEY")) &&
  !isPlaceholder(env("MPESA_CONSUMER_SECRET"))
);
const isValidTransactionType = (value) => VALID_TRANSACTION_TYPES.has(String(value || "").trim());
...
const isConfigured = () => {
  ...
  return enabled && isDarajaConfigured() && Boolean(
    ...
    isValidTransactionType(env("MPESA_TRANSACTION_TYPE", "CustomerPayBillOnline")) &&
    ["production", "sandbox"].includes(env("MPESA_ENVIRONMENT", "production").toLowerCase())
  );
};
...
async function stkPush({ phoneNumber, amount, accountReference, transactionDesc, requestId = "" }) {
...
const getConfigurationSummary = () => {
  ...
  if (!isValidTransactionType(transactionType)) warnings.push("Unsupported MPESA_TRANSACTION_TYPE; use CustomerPayBillOnline or CustomerBuyGoodsOnline.");
  return {
...
module.exports = { isConfigured, isValidTransactionType, timestamp, maskPhone, isB2CConfigured, isDarajaConfigured, normalizePhone, normalizeAccountReference, stkPush, b2cPayment, idempotencyKey, getConfigurationSummary, endpointSummary, extractUpstreamError };
